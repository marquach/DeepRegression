...
test_that("subnetwork_init", {
  ...
  res <- suppressWarnings(subnetwork_init_torch(list(pp)))
  expect_true("nn_module" %in% class(res[[2]]))
...
test_that("shared layer within formula", {
  ...
  param_nr = 1L
  d = dnn_placeholder_processor(function() nn_sequential( 
    nn_linear(in_features = 1, out_features = 5),
    nn_linear(in_features = 5, out_features = 1)
    ))
  specials = c("s", "te", "ti", "vc", "lasso", "ridge", "offset", "vi", "fm", "vfm")
  specials_to_oz = c("d")
  controls$gamdata <- precalc_gam(list(form), data, controls)
