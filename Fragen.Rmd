...
- Alles bis zu return_prepoc = T kann verwendet bzw. muss nicht verändert werden
  ...
  - Output sind dann die parsed_formulas_contents (Output ist in test_data abgespeichert) -> DR: was meint das?
...
-> DR: Das sind Optionen, die jeder einzelner generator (bspw. der gam_processor selbst definieren kann)
  - return(do.call(layer_class, layer_args)(x)) hat nur "x" als input
  - nochmal genauer anschauen (Stichwort: "enviroments") -> DR: versteh ich nicht ganz
  - sollte aus parent.env(environment()) kommen (nicer trick)
    - parent.env(environment())\$layer_args,
    - parent.env(environment())\$layer_class
	Drüber steht Käse. layer_generator hat als default dense.layer genutzt
...
- Fragen zu deepregression:::process_terms:
  ...
  - procs beinhaltet prozessoren für alle specials(=splines, lasso, ...) -> DR: Richtig
  - control und alle processors werden args hinzugefügt -> DR: Richtig
  - Prozessor vom jeweiligen special wird mit args genutzt genutzt -> DR: Richtig
  - Jedes submodul hat eigenen Datensatz (data_trafo()) -> DR: Richtig
    - also Intercept lauter 1er, s(xa) hat xa, deep_model(x1, x2, x3) hat x1, x2, x3 usw.
    - gleiches dann nochmal für anderen Verteilungsparameter -> DR: Richtig
