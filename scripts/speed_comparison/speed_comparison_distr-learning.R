...
setting_res <- lapply(X = 1:nrow(settings), function(x){
  ...
  results <- cbind(
    "plain_loop" = rev(lapply(model$parameters, as.array)[1:2]),
...
save(setting_res, file = "scripts/speed_comparison/speed_comparison_distr-learning.RData")
setting_res
...
ggplot(data = setting_res_frame, aes(x = Approaches, y = elapsed, fill = Approaches))+
  ...
  theme_classic()+ ylab("Elapsed Time") + xlab("Approaches")+
